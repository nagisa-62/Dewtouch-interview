					...
					if(!empty($this->request->data['MigrationFile']['file']['name'])) {
						...
						if (is_file($file['tmp_name'])){
							...
							if ($extension == 'xlsx') {
								...
								$sheetData = $excelSheet->getActiveSheet()->toArray(null,true,true,true);
								...
								for ($i = 2; $i <= $total; $i++) {
									...
									$transactionArray[$i]['date'] =  date('Y-m-d' ,strtotime($sheetData[$i]['A']));
									...
									$transactionItemArray[$i]['description'] = "Being Payment for:".$sheetData[$i]['K'].":".$sheetData[$i]['L'];
									$transactionItemArray[$i]['quantity'] = 1;
									$transactionItemArray[$i]['unit_price'] = $sheetData[$i]['M'];
									$transactionItemArray[$i]['sum'] = $transactionItemArray[$i]['quantity'] * $transactionItemArray[$i]['unit_price'];
									$transactionItemArray[$i]['table'] = "Member";
									$transactionItemArray[$i]['table_id'] = $i-1;
									
								}
								if (!$this->Member->saveAll($memberArray)){
									$this->Session->setFlash('Error insert members table','default',array('class' => 'alert alert-danger'),'result');
								}
								else if (!$this->Transaction->saveAll($transactionArray)){
									$this->Session->setFlash('Error insert transactions table','default',array('class' => 'alert alert-danger'),'result');
								}
								else if (!$this->TransactionItem->saveAll($transactionItemArray)){
									$this->Session->setFlash('Error insert transaction_items table','default',array('class' => 'alert alert-danger'),'result');
								}
								else {
									$this->Session->setFlash('Migrated successfully','default',array('class' => 'alert alert-success'),'result');
								}
							}
							else {
								$this->Session->setFlash('File is NOT xlsx file','default',array('class' => 'alert alert-danger'),'result');
							}
						}
					}
				}
